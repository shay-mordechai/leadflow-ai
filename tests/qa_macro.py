...
def run_macro_qa():
    ...
    try:
        ...
        if reg_res.status_code == 201:
            ...
            sys.exit(log("AUTH", f"Unexpected registration status: {reg_res.status_code} - {reg_res.text}", "FAIL"))
    except RequestException as e:
        sys.exit(log("AUTH", f"Registration connection error: {e}", "FAIL"))

    # --- 2. LOGIN (Triggers OTP) ---
    log("AUTH", "Attempting login to trigger OTP...", "INFO")
    try:
        login_res = requests.post(
            f"{base_url}/api/v1/auth/login", data={"username": email, "password": password}, timeout=LOGIN_TIMEOUT
        )
        ...
        sys.exit(log("AUTH", f"Login error: {e}", "FAIL"))
    ...
    requests.post(f"{base_url}/api/v1/billing/redeem-coupon", json={"coupon_code": "VIP_SHAY"}, headers=headers, timeout=REQ_TIMEOUT)
    log("BILLING", "✅ Account promoted to PRO.", "SUCCESS")

    # --- 5. WEBHOOK & IDEMPOTENCY ---
    test_phone = f"+97254{ts % 10000000}" 
